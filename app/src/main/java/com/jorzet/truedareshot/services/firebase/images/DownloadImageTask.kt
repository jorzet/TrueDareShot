...
package com.jorzet.truedareshot.services.firebase.images
...
import com.jorzet.truedareshot.services.firebase.DownloadImages
import java.io.*
...
class DownloadImageTask(context : Context): AbstractRequestTask<Any, Void, String>() {
    ...
    override fun onPostExecute(result: String) {
        ...
        if (result.equals(DOWNLOAD_COMPLETE)) {
            ...
            val error = GenericError(ErrorType.CANNOT_DOWNLOAD_CONTENT, "")
            onRequestLietenerFailed.onFailed(error)
        }
    ...
    private fun downloadToLocalFile(imageName : String) {
        ...
        val fileRef = storage.getReference().child("/images/${imageName}")
        if (fileRef != null) {
            try {
